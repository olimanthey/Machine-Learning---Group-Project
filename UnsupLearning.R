...
clustering_vars <- marine_db %>%
  select(fuel_consumption_per_hour, exhaust_temp, engine_temp, vibration_level, running_period) %>%
  scale()
...
# K-mean clustering
set.seed(123)
kmeans_result <- kmeans(clustering_vars, centers = 5, nstart = 25)
...
vars_of_interest <- c("fuel_consumption_per_hour", "exhaust_temp", "engine_temp", "vibration_level", "running_period")
